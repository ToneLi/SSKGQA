...
parser.add_argument('--n_epochs', type=int, default=2, help='input batch size')
...
def TE_BERT(bert, test_data):
  ...
  for doub in tqdm(test_data):
    ...
    if predict in right_labels:
      right = right + 1
  print("the result is", right / 1815) # becauce the all sample in the test is 1815, we can not /len(test_data)
  return  right / 1815
...
if __name__=="__main__":
  train_data_path='webspq_train_graph.txt'
  test_data_path="webseb_SS_step1_test_87_end_orgnized.txt"
