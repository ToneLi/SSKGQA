...
def get_input_id():
    ...
    validation_sampler = SequentialSampler(validation_data) # 不打乱
...
#
        ...
        if np.float64(global_results)>0.869:
            checkpoint_path = 'checkpoints/'
            print("the current is bigger than 83")
            print("the result is",global_results)
            torch.save(best_model, checkpoint_path + "best_score_model.pt")
            exit()
